...
public abstract class GLInterface implements IInterface {
	...
	public void drawString (Point pos, int height, String s, Color c) {
		...
		double scale=height/(double)Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT;
		GlStateManager.scale(scale,scale,1);
		end(false);
		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(s,0,0,c.getRGB());
		begin(false);
	...
	public int getFontWidth (int height, String s) {
		double scale=height/(double)Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT;
		return (int)Math.round(Minecraft.getMinecraft().fontRendererObj.getStringWidth(s)*scale);
	}
